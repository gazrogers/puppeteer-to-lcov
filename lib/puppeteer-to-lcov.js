...
module.exports = class PuppeteerToLcov {
	...
	constructor (testName, coverage, config) {
	...
	lcovifyData(data) {
		...
		for(var file in data.files) {
			...
			for(var branch in data.files[file].branches) {
				var br = data.files[file].branches[branch];
				output.push("BRDA:" + br.line + "," + br.block + "," + br.branch + "," + br.hit);
				branchCount += 1;
	...
	getLineCoverage(text, ranges) {
		...
		.map((element, index) => {
			var coveredFlag = this.rangeCheck(ranges, element[0], element[1] - 1, 'containsRange');
	...
	processFunction(path, ranges) {
		...
		var hitFlag = this.rangeCheck(ranges, path.node.body.start, path.node.body.end - 1, 'overlaps');
	...
	getBranchCoverage(parsedCode, ranges) {
		...
		traverse.default(parsedCode, {
			...
			LogicalExpression: function(path) {
				output.push(_this.processLogical(path, ranges));
			}
		});
	...
	processChoice(path, ranges) {
		var consequentHitFlag = this.rangeCheck(ranges, path.node.consequent.start, path.node.consequent.end - 1, 'overlaps');
		...
		if(path.node.alternate) {
			var alternateHitFlag = this.rangeCheck(ranges, path.node.alternate.start, path.node.alternate.end - 1, 'overlaps');
		} else {
	...
	processCase(path, ranges) {
		var hitFlag = this.rangeCheck(ranges, path.node.start, path.node.end - 1, 'overlaps');
	...
	processLogical(path, ranges) {
		var hitFlag = this.rangeCheck(ranges, path.node.right.start, path.node.right.end - 1, 'overlaps');

		return {
			line: path.node.right.loc.start.line,
			block: path.node.right.start,
			branch: 0,
			hit: hitFlag ? 1 : 0
		};
	}
	...
	rangeCheck(ranges, rangeStart, rangeEnd, rangeMethod) {
		return ranges.map(range => {
			return range[rangeMethod](rangeStart, rangeEnd);
		})
