...
module.exports = class PuppeteerToLcov {
	...
	output () {
		...
		{
			var include = this.config.exclude.reduce((result, element) => {
				return result && !minimatch(this.coverage[index].url, element);
			}, true);
			if(include) {
				output += this.getFileInformation(this.coverage[index]);
			}
			this.getAstCoverageInfo(this.coverage[index].text, this.coverage[index].ranges);
		}
		return output;
	}
	...
	getAstCoverageInfo(text, ranges) {
		...
		traverse.default(parsedCode, {
			FunctionDeclaration: function (path) {console.log(path);}
		});
